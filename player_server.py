...
def resolve_stream_url(video_id):
    """Get a direct stream URL and title for a single video.
...
def pick_and_resolve(playlist_id, exclude_id=None):
    ...
    url, title = resolve_stream_url(video_id)
    return {"url": url, "video_id": video_id, "title": title}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/resolve-video":
            ...
            try:
                url, title = resolve_stream_url(video_id)
                return json_response(self, {"url": url, "video_id": video_id, "title": title})
            except Exception as e:
